...
func (q *Queries) GetOrganizationBookings(ctx context.Context, organizationID string) ([]GetOrganizationBookingsRow, error) {
	rows, err := q.db.Query(ctx, getOrganizationBookings, organizationID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
